...
struct Bus: sc_module
{
  ...
  SC_CTOR(Bus)
  {
    ...
    {
      char txt[20];
      snprintf(txt, sizeof(txt), "targ_socket_%u", i);
    ...
    {
      char txt[20];
      snprintf(txt, sizeof(txt), "init_socket_%u", i);
  ...
  //   bits [7:6] select the target, bits [5:0] are the local address
  ...
  {
    return (sc_dt::uint64(target_nr) << 6) | (address & 0x3F);
  }
