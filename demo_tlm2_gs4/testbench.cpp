...
// Both components are able to accept transactions on the return path, although neither
// component actually uses the return path (TLM_UPDATED).

// Shows the Approximately Timed coding style.
// Models processing delay of initiator, latency of target, and request/response accept delays.
// Uses payload event queues to manage both timing annotations and internal delays.

// Shows the BEGIN_REQ exclusion rule at the initiator and the BEGIN_RESP exclusion rule at
// the target. In this example, the target allows two pipelined transactions in-flight.
...
int sc_main(int argc, char* argv[])
{
    Top top("top");
    sc_start();
    cout << "Done -- approximately-timed protocol trace written to foo.txt" << endl;
    return 0;
}
