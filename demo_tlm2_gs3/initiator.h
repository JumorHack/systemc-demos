...
struct Initiator: sc_module
{
    ...
    void thread_process()
    {
        ...
        // Generate a random sequence of reads and writes across a target boundary
        for (int i = 256 - 64; i < 256 + 64; i += 4)
        {
            ...
            // Use DMI if it is available *and* covers this system address
            if (dmi_ptr_valid
            ...
            else
            {
                ...
                {
                    char txt[100];
                    snprintf(txt, sizeof(txt), "Error from b_transport, response status = %s",
                ...
                {
                    // Reuse transaction object for DMI. Reset the address because it may
                    // have been masked by the interconnect on the previous transport call.
                    trans->set_address( i );
        ...
        // Use debug transaction interface to dump two targets' contents
        sc_dt::uint64 A = 128;
        trans->set_address(A);
        trans->set_read();
